...
export class CreateReportComponent implements OnInit {
  ...
  isFormInvalid(): boolean {
    ...
    // Bắt buộc reCAPTCHA phải được giải
    if (!this.requestPayload.captchaToken) {
      return true;
    }

    // Nới lỏng các điều kiện khác, chỉ kiểm tra các trường bắt buộc cơ bản của Angular
    // Tức là, nếu một trường có `required` và chưa điền, form vẫn invalid.
    // Các validation phức tạp khác như moneyScam > 10000, trim() sẽ không ảnh hưởng đến trạng thái disabled của nút.
  ...
  createReport(): void {
    ...
    if (this.isFormInvalid()) {
      // Thông báo lỗi nếu captcha chưa được tick hoặc các trường required cơ bản chưa điền
